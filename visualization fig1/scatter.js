...
d3.csv("updated_centroids.csv", function(data) {
  data.forEach(function(d) {
    d[id] = + d[id];
    d[vx] = +d[vx];
    d[vy] = +d[vy];
    d[count] = +d[count];
    d[frequent_words] = d[frequent_words];
  });

  var xMax = d3.max(data, function(d) { return d[vx]; }) * 1.05,
  ...
  var tip = d3.tip()
      ...
      .offset([-10, 0])
      .html(function(d) {
        return frequent_words + ": <br>" + d[frequent_words];
      });
  ...
  function change() {
    vx = "vx";
    xMax = d3.max(data, function(d) { return d[vx]; });
    xMin = d3.min(data, function(d) { return d[vx]; });
